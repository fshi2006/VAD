...
if not use_offline_img_feat: #resetNet: 512
    img_feat_channel=64
    offline_feature_resize_shape=(35, 60)
...
train_pipeline = [
    ...
    dict(
        ...
        keys=['img_inputs','points', 'gt_bboxes_3d', 'gt_labels_3d', 
              'gt_depth', 'raw_img', 'canvas', 'ego_pos', 'timestamp', 'gt_bboxes_velocity'])
]
...
data = dict(
    ...
    train=dict(
        ...
        split='training',
            ...
            box_type_3d='LiDAR',
            file_client_args=file_client_args),
    val=dict(type=plus_dataset_type,
             data_root=data_root,
             pc_range=point_cloud_range,
    ...
    test=dict(type=plus_dataset_type,
              data_root=data_root,
              pc_range=point_cloud_range,
              ann_file=data_root + 'vad_nuscenes_infos_temporal_val.pkl',
              pipeline=test_pipeline, bev_size=(bev_h_, bev_w_),
